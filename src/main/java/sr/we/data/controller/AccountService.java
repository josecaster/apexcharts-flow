...
//        HttpEntity<String> httpEntity = getAuthHttpEntity(body,accessToken);
        ...
        return encapsulate(() -> {
            ...
            Type collectionType = new TypeToken<PagingResult<Account>>() {
            }.getType();
            PagingResult<Account> myJson = new GsonBuilder().registerTypeAdapter(LocalDate.class, new LocalDateAdapter()).registerTypeAdapter(byte[].class, new ByteArrayAdapter()).registerTypeAdapter(LocalDateTime.class, new LocalDateTimeAdapter()).create().fromJson(content, collectionType);
            return myJson;
        });
    }

    public Account createOrEdit(String accessToken, AccountVO vo) {


        return encapsulate(() -> {
            String body = new GsonBuilder().create().toJson(vo);
            RestTemplate restTemplate = new RestTemplate();
            String fooResourceUrl = configProperties.getRest() + (vo.isNew() ? Routes.ACCOUNT_CREATE : Routes.ACCOUNT_EDIT);
            HttpEntity<String> httpEntity = getAuthHttpEntity(body, accessToken);
    ...
    public Long delete(String accessToken,AccountVO vo) {
        return encapsulate(() -> {
            String body = new GsonBuilder().create().toJson(vo);
            RestTemplate restTemplate = new RestTemplate();
            String fooResourceUrl = configProperties.getRest() + Routes.ACCOUNT_DELETE;
            HttpEntity<String> httpEntity = getAuthHttpEntity(body, accessToken);
